#!/usr/bin/env bash
#
# run_socket_buffer_txrx_matrix.sh
#
# W08-7 / Issue #131 follow-up:
# SO_RCVBUF と SO_SNDBUF の組み合わせが missing / latency に与える影響を確認する。
# 受信 buffer は 8000 / 12000 / 16000、送信 buffer は 2000..16000 を見る。
...
RUN_ROOT="data/w08/socket_buffer_txrx_matrix"
LOG_ROOT="logs/w08/socket_buffer_txrx_matrix"
...

{
  ...
  echo "purpose=socket_buffer_txrx_matrix"
    ...
    for sndbuf in "${SNDBUFS[@]}"; do
      for trial in "${TRIALS[@]}"; do
        ...
        rx_cmd=(./bin/rx
          ...
          --rcvbuf "$rcvbuf"
          --link-name w08_socket_buffer_txrx_matrix
          --trial "$trial"
