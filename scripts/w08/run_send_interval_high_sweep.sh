#!/usr/bin/env bash
#
# run_send_interval_high_sweep.sh
#
# W08-6 の拡張 sweep:
# `--rate-hz` を高めの値に振って、latency と CPU 使用率の余地を確認する。
#
# 対象 rate_hz:
#   50000 / 100000 / 120000 / 140000 / 160000 / 180000 / 200000 / 250000 / 500000 / 1000000
#
# baseline と同じ loopback 条件を維持しつつ、--rate-hz だけを変えて
# 各 rate を 3 trial 実行する。
...
RUN_ROOT="data/w08/send_interval_high"
LOG_ROOT="logs/w08/send_interval_high"

RATES=(50000 100000 120000 140000 160000 180000 200000 250000 500000 1000000)
TRIALS=(1 2 3)
...

{
  ...
  echo "purpose=high_rate_send_interval_sweep"
  echo
} >> "$RUN_ROOT/run_metadata.md"
...
for rate in "${RATES[@]}"; do
  for trial in "${TRIALS[@]}"; do
...

    {
      ...
      echo "rx_cmd=./bin/rx --bind-ip 127.0.0.1 --port 9000 --duration-sec 12 --log-path $run_dir/rx.log --link-name w08_send_interval_high --trial $trial --csv-in-1sec-log-path $run_dir/rx_1sec.csv --csv-by-1recv-log-path $run_dir/rx_by_1recv.csv --recovery-mode fsm"
    ...
    ./bin/rx \
      ...
      --link-name w08_send_interval_high \
      --trial "$trial" \
